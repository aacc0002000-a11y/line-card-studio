...
function upsertCardSettings_(record) {
  ...
  var targetRow = findRecordRowNumber_(records, 'card_id', record.card_id);
...
function replaceCardButtons_(cardId, buttons) {
  ...
  for (index = 0; index < allRecords.length; index += 1) {
    if (String(allRecords[index].card_id) !== String(cardId)) {
  ...
  if (rowsToKeep.length) {
    writeRecordsToSheet_(sheet, headers, rowsToKeep);
  }

  if (buttons && buttons.length) {
    writeRecordsToSheet_(sheet, headers, buttons);
  }
}
...
function replaceCardAssets_(cardId, assets) {
  ...
  if (assets && assets.length) {
    writeRecordsToSheet_(sheet, headers, assets);
  }
}
